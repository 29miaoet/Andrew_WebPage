...
class SupabaseClient {
  ...
  async select(table, query = "*") {
    return this.request(
      "GET",
      `/${table}?select=${query}&order=created_at.desc`,
    );
  }
  ...
  try {
    ...
    if (!data || data.length === 0) {
      signaturesContainer.innerHTML =
        '<div class="no-signatures">Be the first to sign!</div>';
    ...
    data.forEach((signature, index) => {
      ...
      const message =
        signature.status === "active" && signature.message
          ? signature.message
          : "";
    ...
    signaturesContainer.innerHTML =
      '<div class="no-signatures">Unable to load signatures</div>';
  }
}
